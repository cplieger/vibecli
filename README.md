...
| `KIRO_CLI_PATH` | Escape hatch: run this kiro-cli binary instead of the one the server installs. Set it and the server stops managing kiro-cli entirely, which also stops `/api/health` from reporting kiro-cli readiness. Leave it unset unless you are supplying your own binary. | _(unset: the server installs the pinned version)_ |
