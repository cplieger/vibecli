...
# cannot drift: names read off the /config bind
...
# can. The ordering is the point: the server this script execs installs packages
# from the network (kiro-cli's archive, and any `apt:` tool entry), and holding
# the broadest capability there is across network-fetched installation to buy a
# single mount call would be a bad trade. Re-executing here reduces the window to
# that one call.
