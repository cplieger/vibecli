...
// HTTP surface and the `docker exec` repair path stay alive, which is the
// repo's failure posture: a broken install degrades readiness, it does not
...
// the session-create gate's 503 reason, which must distinguish "still
// installing" from "retrying" from "gave up". Ready is the authority on
// whether sessions may start; Phase only explains why they may not.
type Phase string
...
const (
	// mainBinary is the dispatcher every version probe and every session
...
type Config struct {
	...
	// complete. Empty defaults to this app's set (the main dispatcher plus
	// the chat sidecar every session dispatches through). mainBinary is
	// always included.
	Required []string
...
// present: readiness probes it and every session runs it, so a caller cannot
// configure it out of the required set.
func withMainBinary(required []string) []string {
	if len(required) == 0 {
		// This app dispatches `kiro-cli chat` into the chat sidecar, so a
		// version directory without it answers --version correctly while
		// every terminal dies at chat. It is part of the SET, not an extra.
		return []string{mainBinary, "kiro-cli-chat"}
	}
...
// Ready reports whether sessions may start, and why not when they may not.
//
...
// reason. Ready, not Phase, decides whether sessions may start.
...
// PathEntry returns the absolute directory to prepend to the session PATH, or
// "" when no version is active. It holds only kiro-cli's own dispatchers, so
// leading with it shadows nothing else.
...
func (m *Manager) recordUnavailable(installErr error) error {
	...
	slog.Error("no usable kiro-cli version is installed; sessions stay gated until one is",
