...
// the required set whatever a caller passes: readiness probes it and every
// session runs it, so a version directory without it must never read complete.
func TestNewAlwaysRequiresTheMainDispatcher(t *testing.T) {
	tests := map[string]struct {
		required []string
		want     []string
	}{
		"empty defaults to this app's set": {required: nil, want: []string{mainBinary, "kiro-cli-chat"}},
