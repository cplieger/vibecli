...
func TestPurgeLegacySparesToolbeltSymlinks(t *testing.T) {
	...
	journal := filepath.Join(env.tools, ".kiro-cli-installed")
	if err := os.WriteFile(journal, []byte("legacy\n"), 0o600); err != nil {
	...
	assertToolbeltIntact(t, env.tools, survivors)
	if exists(journal) {
		t.Error("the legacy sweep no longer removes the shell installer's own artifacts")
	}
}
