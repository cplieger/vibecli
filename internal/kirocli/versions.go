...
// image rebuild, which recreates the container and therefore ends every live
// session, so no session can outlive two version changes and no running process
// can still hold a reference into a directory this function drops. If in-process
// LIVE upgrades are ever enabled — a new pin reaching a running container
// without a recreate — that argument dies: a session started on version A could
// still need to dispatch a sidecar out of A's directory, and pruning would then
// have to take a per-version lease from every live session and remove a
// directory only at zero leases. Revisit this function first if that changes.
...
// into a mass delete would throw away the fallback set the failure posture
// depends on.
