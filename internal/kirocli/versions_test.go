...
func TestVersionDirCompleteRejectsEveryIncompleteShape(t *testing.T) {
	tests := map[string]struct {
		...
		"required dispatcher is a symlink": {
			setup: func(t *testing.T, dir string) {
				writeSet(t, dir, pinnedVersion, "kiro-cli")
				...
				if err := os.Symlink(target, filepath.Join(dir, "kiro-cli-chat")); err != nil {
					t.Fatalf("Symlink: %v", err)
				}
				writeSentinelFile(t, dir, pinnedVersion)
			},
		},
		"required dispatcher is not executable": {
			setup: func(t *testing.T, dir string) {
				writeSet(t, dir, pinnedVersion, "kiro-cli")
				if err := os.WriteFile(filepath.Join(dir, "kiro-cli-chat"), []byte("x"), 0o600); err != nil {
